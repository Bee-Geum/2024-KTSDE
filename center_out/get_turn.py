...
def get_paths(cities, path_name, k, option):
    ...
    for city in cities:
        for num in samples:
            with open(f'./{dir}/{city}_{option}_{path_name}_{num}_{k}.pkl', 'rb') as file:
...
def get_turn(tup):
    city, num, paths, G, path_name, k, option = tup
    ...
    with open(f'./result/{city}_{option}_{path_name}_turns_{num}_{k}.pkl', 'wb') as file:
        pickle.dump(turns, file)



def main(k, path_name, txt_file, option):
    ...
    paths_data = get_paths(dict_G, path_name, k, option)
    ...
    for city in cities:
        ...
        for num in sample_num:
            ...
            file_infos.append((city, num, paths, G, path_name, k, option))
